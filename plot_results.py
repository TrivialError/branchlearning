...
def plot(SB_runtime, SB_nodes, Random_runtime, Random_nodes, Objective_runtime, Objective_nodes, Fractional_runtime, Fractional_nodes):  
    plt.figure(1)    
    plt.scatter([row[0] for row in SB_runtime], [row[1] for row in SB_runtime], color='red')
    plt.scatter([row[0] for row in Random_runtime], [row[1] for row in Random_runtime], color='blue')
    plt.scatter([row[0] for row in Objective_runtime], [row[1] for row in Objective_runtime], color='green')
    plt.scatter([row[0] for row in Fractional_runtime], [row[1] for row in Fractional_runtime], color='purple')
    ...
    plt.figure(2)    
    plt.scatter([row[0] for row in SB_nodes], [row[1] for row in SB_nodes], color='red')
    plt.scatter([row[0] for row in Random_nodes], [row[1] for row in Random_nodes], color='blue')
    plt.scatter([row[0] for row in Objective_nodes], [row[1] for row in Objective_nodes], color='green')
    plt.scatter([row[0] for row in Fractional_nodes], [row[1] for row in Fractional_nodes], color='purple')
    #plt.scatter([row[0] for row in LB_runtime], [row[1] for row in LB_runtime], color='black')
    plt.xlabel("Number of Nodes")
